...
export default function ScanPage() {
  ...
  async function handleScan() {
    ...
    try { const r = await triggerScan(token); setScanMessage(r.message); startPolling(); }
    catch { setScanMessage("Failed to start scan."); setScanning(false); }
  }
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={handleScan} disabled={scanning} className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 disabled:bg-gray-700 text-white font-medium rounded-lg transition-colors flex items-center gap-2">
            {scanning ? (<><span className="inline-block animate-spin">⟳</span> Scanning... {elapsed > 0 && `${elapsed}s`}</>) : "Run Scan"}
          </button>
        </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {displayResults.map((r) => {
                  ...
                  return (
                    <tr key={key} onClick={() => router.push(`/scan/detail/${r.rank}`)} className="border-b border-gray-800/50 hover:bg-gray-900/50 transition-colors cursor-pointer">
                      <td className="py-3 pr-2"><button onClick={(e) => handleStar(r, e)} className={`text-lg transition-colors ${isStar ? "text-yellow-400" : "text-gray-600 hover:text-yellow-400"}`}>{isStar ? "★" : "☆"}</button></td>
                      <td className="py-3 pr-3 text-gray-500">{r.rank}</td>
                      <td className="py-3 pr-3 font-semibold">{r.ticker}</td>
                      <td className="py-3 pr-3"><span className={`px-2 py-0.5 rounded text-xs font-medium ${r.strategy === "COVERED_CALL" ? "bg-blue-900/50 text-blue-300" : "bg-purple-900/50 text-purple-300"}`}>{r.strategy === "COVERED_CALL" ? "CC" : "CSP"}</span></td>
                      <td className="py-3 pr-3 text-right">${r.strike.toFixed(2)}</td>
                      <td className="py-3 pr-3 text-right text-gray-400">{r.expiry}</td>
                      <td className="py-3 pr-3 text-right">{r.dte}</td>
                      <td className="py-3 pr-3 text-right">${r.premium.toFixed(2)}</td>
                      <td className="py-3 pr-3 text-right">{r.delta.toFixed(3)}</td>
                      <td className="py-3 pr-3 text-right">${r.theta.toFixed(3)}</td>
                      <td className="py-3 pr-3 text-right">{(r.iv * 100).toFixed(1)}%</td>
                      <td className="py-3 pr-3 text-right">{(r.ann_return * 100).toFixed(1)}%</td>
                      <td className="py-3 text-right"><span className={`font-bold ${r.score >= 70 ? "text-emerald-400" : r.score >= 50 ? "text-yellow-400" : "text-red-400"}`}>{r.score.toFixed(1)}</span></td>
                    </tr>
                  );
                })}
              </tbody>
